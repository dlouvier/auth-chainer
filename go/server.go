...
func main() {
	...
	sessionCookieSecret, envVarSet := os.LookupEnv("SESSION_COOKIE_SECRET")
	if !envVarSet {
		log.Fatalln("SESSION_COOKIE_SECRET enviroment variable is not set.")
	}

	e.Use(session.Middleware(sessions.NewCookieStore([]byte(sessionCookieSecret))))
